...
import productVideo from "../Product/product4.mp4";
...
export default function Auckland() {
  ...
  const callFunc = () => {
    ...
    if (!isFuncCalled && !cooldownActive) {
      const updatedData = {
        productId: "664d89ee7f79eb1ab87a58fe",
        call: true,
      };
      ...
      axios
        ...
        .then((response) => {
          ...
          localStorage.setItem("lastCallTimeAuckland", Date.now());
        })
        .catch((error) => {
  ...
  useEffect(() => {
    updateAmount();
    const lastCallTimeAuckland = localStorage.getItem("lastCallTimeAuckland");
    const currentTime = Date.now();
    if (lastCallTimeAuckland && currentTime - lastCallTimeAuckland < 14400000) {
      // Cooldown is still active, set cooldown active state to true
      setCooldownActive(true);
      // Set timeout for remaining cooldown time
      setTimeout(
        () => setCooldownActive(false),
        14400000 - (currentTime - lastCallTimeAuckland)
      );
    }
  }, []);
