...
import productVideo from "../Product/product2.mp4";
...
export default function Adani() {
  ...
  const callFunc = () => {
    ...
    if (!isFuncCalled && !cooldownActive) {
      const updatedData = {
        productId: "664d88bbe25ae447914460df",
        call: true,
      };
      ...
      axios
        ...
        .then((response) => {
          ...
          localStorage.setItem("lastCallTimeAdani", Date.now());
        })
        .catch((error) => {
  ...
  useEffect(() => {
    updateAmount();
    const lastCallTimeAdani = localStorage.getItem("lastCallTimeAdani");
    const currentTime = Date.now();
    if (lastCallTimeAdani && currentTime - lastCallTimeAdani < 14400000) {
      // Cooldown is still active, set cooldown active state to true
      setCooldownActive(true);
      // Set timeout for remaining cooldown time
      setTimeout(
        () => setCooldownActive(false),
        14400000 - (currentTime - lastCallTimeAdani)
      );
    }
  }, []);
