...
export default function Product1() {
  ...
  const callFunc = () => {
    ...
    if (!isFuncCalled && !cooldownActive) {
      const updatedData = {
        productId: "664d892de25ae447914460e1",
        call: true,
      };
      ...
      axios
        ...
        .then((response) => {
          ...
          localStorage.setItem("lastCallTimeProduct1", Date.now());
        })
        .catch((error) => {
  ...
  useEffect(() => {
    updateAmount();
    const lastCallTimeProduct1 = localStorage.getItem("lastCallTimeProduct1");
    const currentTime = Date.now();
    if (lastCallTimeProduct1 && currentTime - lastCallTimeProduct1 < 14400000) {
      // Cooldown is still active, set cooldown active state to true
      setCooldownActive(true);
      // Set timeout for remaining cooldown time
      setTimeout(
        () => setCooldownActive(false),
        14400000 - (currentTime - lastCallTimeProduct1)
      );
    }
  }, []);
