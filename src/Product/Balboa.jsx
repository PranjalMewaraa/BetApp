...
import productVideo from "../Product/product5.mp4";
...
export default function Balboa() {
  ...
  const callFunc = () => {
    ...
    if (!isFuncCalled && !cooldownActive) {
      const updatedData = {
        productId: "664d8a047f79eb1ab87a5901",
        call: true,
      };
      ...
      axios
        ...
        .then((response) => {
          ...
          localStorage.setItem("lastCallTimeBalboa", Date.now());
        })
        .catch((error) => {
  ...
  useEffect(() => {
    updateAmount();
    const lastCallTimeBalboa = localStorage.getItem("lastCallTimeBalboa");
    const currentTime = Date.now();
    if (lastCallTimeBalboa && currentTime - lastCallTimeBalboa < 14400000) {
      // Cooldown is still active, set cooldown active state to true
      setCooldownActive(true);
      // Set timeout for remaining cooldown time
      setTimeout(
        () => setCooldownActive(false),
        14400000 - (currentTime - lastCallTimeBalboa)
      );
    }
  }, []);
