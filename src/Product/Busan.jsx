...
export default function Busan() {
  ...
  const callFunc = () => {
    ...
    if (!isFuncCalled && !cooldownActive) {
      const updatedData = {
        productId: "664d8a277f79eb1ab87a5907",
        call: true,
      };
      ...
      axios
        ...
        .then((response) => {
          ...
          localStorage.setItem("lastCallTimeBusan", Date.now());
        })
        .catch((error) => {
  ...
  useEffect(() => {
    updateAmount();
    const lastCallTimeBusan = localStorage.getItem("lastCallTimeBusan");
    const currentTime = Date.now();
    if (lastCallTimeBusan && currentTime - lastCallTimeBusan < 14400000) {
      // Cooldown is still active, set cooldown active state to true
      setCooldownActive(true);
      // Set timeout for remaining cooldown time
      setTimeout(
        () => setCooldownActive(false),
        14400000 - (currentTime - lastCallTimeBusan)
      );
    }
  }, []);
