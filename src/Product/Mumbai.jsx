...
export default function Mumbai() {
  ...
  const callFunc = () => {
    ...
    if (!isFuncCalled && !cooldownActive) {
      const updatedData = {
        productId: "664d8829e25ae447914460dd",
        call: true,
      };
      ...
      axios
        ...
        .then((response) => {
          ...
          localStorage.setItem("lastCallTimeMumbai", Date.now());
        })
        .catch((error) => {
  ...
  useEffect(() => {
    updateAmount();
    const lastCallTimeMumbai = localStorage.getItem("lastCallTimeMumbai");
    const currentTime = Date.now();
    if (lastCallTimeMumbai && currentTime - lastCallTimeMumbai < 14400000) {
      // Cooldown is still active, set cooldown active state to true
      setCooldownActive(true);
      // Set timeout for remaining cooldown time
      setTimeout(
        () => setCooldownActive(false),
        14400000 - (currentTime - lastCallTimeMumbai)
      );
    }
  }, []);
