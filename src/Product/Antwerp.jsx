...
import productVideo from "../Product/product3.mp4";
...
export default function Antwerp() {
    ...
    const updateAmount = async () => {
        const res = await axios.get("https://kdm-money-server.onrender.com/api/v1/auth/user-details");
    ...
    const callFunc = () => {
        ...
        if (!isFuncCalled && !cooldownActive) {
            const updatedData = {
                productId: "664d8946e25ae447914460e3", 
                call: true
            };

            axios.post("https://kdm-money-server.onrender.com/api/v1/auth/take-action", updatedData)
                .then(response => {
                    ...
                    localStorage.setItem("lastCallTimeAntwerp", Date.now());
                })
                .catch(error => {
    ...
    useEffect(() => {
        updateAmount();
        const lastCallTimeAntwerp = localStorage.getItem("lastCallTimeAntwerp");
        const currentTime = Date.now();
        if (lastCallTimeAntwerp && currentTime - lastCallTimeAntwerp < 14400000) {
            // Cooldown is still active, set cooldown active state to true
            setCooldownActive(true);
            // Set timeout for remaining cooldown time
            setTimeout(() => setCooldownActive(false), 14400000 - (currentTime - lastCallTimeAntwerp));
        }
    }, []);
    ...
    useEffect(() => {
        const intervalId = setInterval(() => {
            ...
            const newMarketCap = increase ? marketCap + randomDelta : marketCap - randomDelta;
